...
def load_folder(folder_path, img_suffix, mask_suffix, desc):
    if not os.path.exists(folder_path):
        print(f"Warning: Folder {folder_path} does not exist. Returning empty dataset.")
        return [], []
    ...
    with tf.compat.v1.Session(config=config_proto) as sess:
        ...
        for epoch in epoch_pbar:
            ...
            if load_from_last and not checkpoint_exists:
                ...
                print(f" -> Restoring model from last checkpoint: {last_epoch_dir}")
            ...
            for _ in batch_pbar:
                try:
                    ...
                    print("Error: Timeout while waiting for batch. Ending training early.")
                    break
