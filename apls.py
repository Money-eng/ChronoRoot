    ...
    def _calculate_single_direction_apls(self, G_source, G_target):
        ...
        for i, u_src in enumerate(tqdm.tqdm(nodes_of_interest)):
            ...
            for j, v_src in enumerate(nodes_of_interest):
                ...
                try:
                    len_src = nx.shortest_path_length(G_source, u_src, v_src,
                                                      weight='length')  # get length in source graph between u_src and v_src
                ...
                try:
                    len_tgt = nx.shortest_path_length(G_target, u_tgt, v_tgt,
                                                      weight='length')  # get length in target graph between u_tgt and v_tgt
...
class TestAPLS_Standardized(unittest.TestCase):
    ...
    def test_09_line_sep(self):
        ...
        self.assertAlmostEqual(res['recall'], expected_recall, places=4,
                               msg="Recall should match expected value for separated graph")
        self.assertAlmostEqual(res['precision'], expected_precision, places=4,
                               msg="Precision should be 1.0 for separated graph")
        self.assertAlmostEqual(res['f1'], expected_f1, places=4,
                               msg="F1 should match expected value for separated graph")
