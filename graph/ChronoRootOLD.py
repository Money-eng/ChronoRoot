...
def analyze_single_component(conf, images, segFiles, bbox, local_seed, root_id):
    ...
    with open(pfile, 'w+') as csv_file:
        ...
        csv_writer.writerow(
            ['FileName', 'TimeStep', 'MainRootLength', 'LateralRootsLength', 'NumberOfLateralRoots', 'TotalLength'])
        ...
        rsmlTree.write(open(os.path.join(rsmlPath, image_name.replace(current_conf['FileExt'], '.rsml')), 'w'),
                       encoding='unicode')
        ...
        for i in range(start + 1, N):
            ...
            if not segErrorFlag and not trackError:
                ...
                if seedrsml is None:
                    ...
                    rsmlTree.write(
                        open(os.path.join(rsmlPath, image_name.replace(current_conf['FileExt'], '.rsml')), 'w'),
                        encoding='unicode')
